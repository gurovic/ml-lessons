            ...
            "cells": [
                {
                    "type": "markdown",
                    "source": (
                        "Сквозной пайплайн на **Titanic (OpenML)**: от сырого `df` до таблицы "
                        "для sklearn — каждый шаг **меняет** `df` и передаёт результат дальше.\n\n"
...
""",
        ...
        {
            "slide_title": "EDA: что в данных?",
            "kind": "eda",
            "cells": [
                {
                    "type": "markdown",
                    "source": "Видим пропуски в Age/Fare и дисбаланс классов. `name` — почти ID, в модель не берём.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Решение: заполнение пропусков",
            "kind": "example",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Решение:** Age — медиана по Pclass; Embarked — мода; Fare — медиана по Pclass. Результат — `df_clean`.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Решение: feature engineering",
            "kind": "example",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Решение:** clip Fare на 99-м перцентиле, бинарный `is_expensive`, отклонение от среднего fare по классу (`transform`).",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "groupby и merge",
            "kind": "viz",
            "cells": [
                {
                    "type": "markdown",
                    "source": "Агрегат по Pclass×Sex и join справочника портов — всё на **df_clean**.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Решение: таблица для модели",
            "kind": "example",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Решение:** OHE для Sex/Embarked, отбираем числовые признаки → `df_model`. Сохраняем в Parquet.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Решение: split и Pipeline",
            "kind": "example",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Решение:** split **после** всех трансформаций; imputer/scaler — только в Pipeline на train.",
                },
                {
                    "type": "code",
                    "source": """\
            ...
            "cells": [
                {
                    "type": "markdown",
                    "source": (
                        "Один датасет **Titanic** — последовательно отвечаем на вопросы графиками. "
                        "Каждый раздел опирается на выводы предыдущего; в конце — простая модель и ROC."
                    ),
                }
            ],
        },
...
""",
        ...
        {
            "slide_title": "Вопрос 1: как распределены Age и Fare?",
            "kind": "viz",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Вывод:** Age — почти нормальный хвост; Fare — сильный перекос, нужен boxplot по классам.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Вопрос 2: связаны ли Age, Fare и выживание?",
            "kind": "viz",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Вывод:** выжившие чаще в 1-м классе и с более высоким Fare; scatter подтверждает.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Вопрос 3: корреляции числовых признаков?",
            "kind": "viz",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Вывод:** Survived слабо коррелирует с Pclass (−) и Fare (+) — согласуется с barplot выше.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Вопрос 4: роль Sex и порта?",
            "kind": "viz",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Вывод:** женщины выживали чаще; Cherbourg — выше доля выживших (согласуется с 1-м классом).",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Вопрос 5: сводная панель EDA",
            "kind": "viz",
            "cells": [
                {
                    "type": "markdown",
                    "source": "**Решение:** собираем ключевые графики в одну фигуру 2×2 — итог истории EDA.",
                },
                {
                    "type": "code",
                    "source": """\
...
""",
        ...
        {
            "slide_title": "Вопрос 6: насколько предсказуемо выживание?",
            "kind": "viz",
            "cells": [
                {
                    "type": "markdown",
                    "source": (
                        "По EDA: Pclass, Fare, Sex — сильные предикторы. "
                        "**Решение:** простая LogReg; один split; ROC на том же test."
                    ),
                },
                {
                    "type": "code",
