...
- В начале ноутбука — **одна** setup-ячейка `# Setup` (импорты, `random_state=42`, `%matplotlib inline`). Сборщик добавляет её автоматически; в секциях — **без повторных** импортов и seed.
