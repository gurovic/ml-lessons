...
| `author_feedback.md` | **автор** | замечания **после** проверки pptx; **постоянный архив** прошлых багов — **не удалять** при перегенерации; агенты читают перед правками; не перезаписывается `lesson_reviewer` |

Рецензент (`lesson_reviewer`) полезен **до** первой сборки pptx. Перед любой перегенерацией урока агенты **читают `author_feedback.md`** (если есть) и учитывают уже исправленные и открытые замечания.

### author_feedback.md — постоянный архив

- Файл **не удалять** при пересборке pptx, JSON или PNG.
- Назначение: история багов и правок после просмотра презентации; при новой генерации — **чеклист «что уже ломалось»**.
- Выполненные пункты помечать `[x]`; новые замечания — дописывать секциями по слайдам.
- Агенты **не перезаписывают** файл целиком (в отличие от `review.md`).
...
- **docs/issue_workflow.md** — опциональные GitHub issue; работа в `main` по умолчанию
...
**Пилот:** правки стиля иллюстраций эталонно проверять в `lessons/lineynaya_regressiya` (`--pilot`); массовая перегенерация — `--all-lessons --generate-only` (см. docs/visuals.md).
