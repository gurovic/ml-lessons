...
- **Импорты один раз за урок:** в первом слайде с `code_examples` — полные `import`/`from`; на следующих слайдах — только тело (предполагаются импорты с предыдущих слайдов или общий контекст урока).
- **Без повторов:** не дублировать `import`, `np.random.seed`, `plt.style` в каждом блоке.
- **Лаконичность:** однострочники sklearn/pandas где уместно; без boilerplate и продвинутого Python (walrus, тяжёлые comprehensions).
...
- Расположение: **под буллетами**, в левой колонке (как текст).
- При наличии `visuals[]` — левая колонка уже сужена (~7.5″); высота блока кода уменьшается, буллеты сжимаются.
- Фон: светло-серый прямоугольник (`#F5F5F5`) вокруг моноширинного текста.
