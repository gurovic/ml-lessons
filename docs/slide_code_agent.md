...
**После** финализации JSON-слайдов (после рецензента и визуализаций), **до** или **после** `pptx_builder` — поле `code_examples` рендерится сборщиком.
...
| `source` | `string` | Python-код, 3–8 строк; переносы `\n` |
| `caption` | `string?` | Подпись под блоком (опционально) |

- Комментарии в коде — **на русском**.
- Без markdown-обёртки ``` в `source`.
- Длина: умещается в нижнюю часть слайда под буллетами.
